...
function jfb_add_plugin_links($links, $file)
{
    if( dirname(plugin_basename( __FILE__ )) == dirname($file) )
        $links[] = '<a href="options-general.php?page=' . "wp-fb-autoconnect" .'">' . __('Settings','wp-fb-ac') . '</a>';
    return $links;
}
...
function jfb_admin_notices()
{
	...
	{
	    ?><div class="error"><p>
	    	<strong><?php _e("Warning", 'wp-fb-ac')?>:</strong> 
	    	<?php echo sprintf(__("This version of WP-FB-AutoConnect is only compatible with Premium addon version 30 or better (you're currently using version %s).  If you haven't done so already, please read the important information in FAQ46 of the plugin documentation:", 'wp-fb-ac'), JFB_PREMIUM_VER) ?> <a target='store' href='<?php echo $jfb_homepage?>/#faq46'><b>here</b></a><br/><br/>
	    	<?php _e("If you'd like to revert to a previous version of the free plugin until you're ready to update the addon, previous versions can be downloaded from:", 'wp-fb-ac') ?> <a target="prev" href='http://wordpress.org/extend/plugins/wp-fb-autoconnect/developers/'><b>here</b></a>
	      </p></div><?php
	}
	...
	{
  	 	?><div class="error"><p><strong><?php _e("Warning", 'wp-fb-ac')?>:</strong> <?php _e("WP-FB-AutoConnect requires the JSON PHP extension to work.  Please install / enable it before attempting to use this plugin.", 'wp-fb-ac')?></p></div><?php
	}
    ...
    {?>
        <div class="error">
            <p><strong><?php _e("Warning", 'wp-fb-ac')?>:</strong> <?php _e("In some situations, W3-Total-Cache has been reported to cause problems with WP-FB-AutoConnect's Facebook logins.  If you're experiencing issues with WP-FB-AutoConnect (for instance, the Widget doesn't properly show the logged-in state immediately after a login), please try temporarily disabling W3-Total-Cache.  If that fixes it, you may re-enable W3-Total-Cache and try selectively enabling its various caching/CDN options until you find a combination that works for you.", 'wp-fb-ac')?></p>
            <?php echo '<a href="?' . http_build_query(array_merge($_GET, array($opt_jfb_hidew3warning => "1"))) . '">'.__("Hide this message", 'wp-fb-ac') . '</a>'; ?>
        </div>
    <?php
    }
...
function jfb_admin_page()
{
    ...
    {
        ?><div class="error"><p><?php _e("Sorry, but as of v1.3.0, WP-FB AutoConnect requires PHP5.", 'wp-fb-ac')?></p></div><?php
        die();
    }
    if( function_exists('is_multisite') && is_multisite() && !jfb_premium() )
    {
        ?><div class="error"><p><strong><?php _e("Warning", 'wp-fb-ac')?>:</strong> <?php _e("Wordpress MultiSite is only fully supported by the premium version of this plugin; please visit the documentation page for more details:", 'wp-fb-ac')?> <a target='no_multisite' href='<?php echo $jfb_homepage?>#premium'><b>here</b></a></p></div><?php
    }
    if( file_exists(realpath(dirname(__FILE__))."/WP-FB-AutoConnect-Premium.php" ) )
    {
        ?><div class="error"><p><strong><?php _e("Notice", 'wp-fb-ac')?>:</strong> 
        	<?php _e("You seem to've uploaded the premium addon to the wrong directory.", 'wp-fb-ac')?><br/><br/>
        	<?php echo sprintf(__("The %s file needs to go in your plugins directory (%s), not the WP-FB-AutoConnect directory (%s). This is to prevent it from getting overwritten when you auto-update the core plugin.", 'wp-fb-ac'), "'WP-FB-AutoConnect-Premium.php'", "i.e. wp-content/plugins/WP-FB-AutoConnect-Premium.php", "i.e. wp-content/plugins/wp-fb-autoconnect/WP-FB-AutoConnect-Premium.php")?>
          </p></div>
        <?php
    }
    ...
    if( isset($_POST['remove_all_settings']) )
    {
        ...
        ?><div class="updated"><p><strong><?php _e('All plugin settings have been cleared.', 'wp-fb-ac' ); ?></strong></p></div><?php
    }
    ?>
    ...
    $tabNames        = array(__("Facebook Setup", 'wp-fb-ac'),
    						 __("Basic Options", 'wp-fb-ac'),
    						 __("Premium Options", 'wp-fb-ac'),
    						 __("Uninstall", 'wp-fb-ac'),
    						 __("Support Info", 'wp-fb-ac'));
    ?>
    ...
    <div class="jfb-admin_wrapper">
		...
		<h3><?php _e("Setup Instructions", 'wp-fb-ac')?></h3>
            ...
            <h3><?php _e("Facebook Connect", 'wp-fb-ac')?></h3>
            <form name="formFacebook" method="post" action="">
                ...
                <div class="submit"><input type="submit" name="Submit" value="<?php _e("Connect", 'wp-fb-ac')?>" /></div>
            </form>
        </div> <!-- End Tab -->
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[1]?>" style="display:<?php echo ($shownTab==1?"block":"none")?>">
            ...
            if(!get_option($opt_jfb_valid))
                echo "<div class=\"jfb-admin_warning\"><i><b>".__("You must enter a valid App ID and Secret under the 'Facebook Setup' tab before this plugin will function.", 'wp-fb-ac')."</b></i></div>";    
            ?>
            <h3><?php _e("Basic Options", 'wp-fb-ac')?></h3>
            <form name="formMainOptions" method="post" action="">
                <b><?php _e("Autoregistered Usernames", 'wp-fb-ac')?>:</b><br />
                <input type="radio" name="<?php echo $opt_jfb_username_style; ?>" value="0" <?php echo (get_option($opt_jfb_username_style)==0?"checked='checked'":"")?> ><?php _e("Based on Facebook ID (i.e. FB_123456)", 'wp-fb-ac')?><br />
                <input type="radio" name="<?php echo $opt_jfb_username_style; ?>" value="1" <?php echo (get_option($opt_jfb_username_style)==1?"checked='checked'":"")?> ><?php _e("Based on real name with prefix (i.e. FB_John_Smith)", 'wp-fb-ac')?><br />
                <input type="radio" name="<?php echo $opt_jfb_username_style; ?>" value="3" <?php echo (get_option($opt_jfb_username_style)==3?"checked='checked'":"")?> ><?php _e("Based on real name without prefix (i.e. John_Smith)", 'wp-fb-ac') . " <i><b>(" . __("Recommended for BuddyPress", 'wp-fb-ac') . ")</b></i>";?><br />
                <input type="radio" name="<?php echo $opt_jfb_username_style; ?>" value="2" <?php echo (get_option($opt_jfb_username_style)==2?"checked='checked'":"")?> ><?php _e("Legacy Format (i.e. John.Smith)", 'wp-fb-ac')?> 
                <i><b>(<?php _e("Not recommended", "wp-fb-ac")?>)</b></i> <?php jfb_output_simple_lightbox("", __("Although the original 'BuddyPress-friendly' username format included a period, I later learned that this creates issues with author links in Wordpress.  I've left the option here for legacy support, but advise against using it (unless you have only one author on your blog, in which case Facebook-connected users won't have author links and so it doesn't matter).  If you do have multiple authors and are experiencing broken author links, changing this option will fix it for all NEW users, but you may want to consider fixing your existing users by replacing all of the '.'s with '_'s in the 'user_nicename' field of the 'wp_users' database table.", 'wp-fb-ac')); ?><br /><br />
            
                <b><?php _e("E-Mail", 'wp-fb-ac')?>:</b><br />
                <input type="checkbox" name="<?php echo $opt_jfb_ask_perms?>" value="1" <?php echo get_option($opt_jfb_ask_perms)?'checked="checked"':''?> /> <?php _e("Request permission to get the connecting user's email address", 'wp-fb-ac')?><br />
        		...
        		<small><i>*<?php _e("Note: In order to use this feature, you'll need to submit your app for review by Facebook.", 'wp-fb-ac')?></i></small>
        		<?php
        		$lbText = __("As a new requirement Facebook introduced in 2014, asking for anything more than a user's basic profile, e-mail, and friends list requires you to submit your app for review.  For more information, see: ", 'wp-fb-ac');
        		$lbText .= " <a target='app-review' href='https://developers.facebook.com/docs/apps/review/login'><b>here</b></a><br/><br/>";
        		$lbText .= sprintf(__("In order to post to your users' timelines, your app will need the '%s' extended permission.  To request it:", 'wp-fb-ac'), "publish_actions")."<br/>";
        		$lbText .= __("1) Visit to your app's configuration page on:", 'wp-fb-ac');
        		$lbText .= " <a target='appconsole' href='https://developers.facebook.com/apps'>developers.facebook.com/apps</a><br />";
        		$lbText .= __("2) Go to 'Status & Review->Start a Submission'", 'wp-fb-ac') . "<br/>";
        		$lbText .= sprintf(__("3) Check '%s,' and proceed with the submission.", 'wp-fb-ac'), "publish_actions") . "<br/><br/>";
        		$lbText .= __("Facebook claims the review process will take 7-14 days, after which this feature will work properly.", 'wp-fb-ac');
        		jfb_output_simple_lightbox("", $lbText);
        		?><br/>
                ...
                <br /><b><?php _e("Appreciation", 'wp-fb-ac')?>:</b><br />
                <input type="checkbox" name="<?php echo $opt_jfb_show_credit?>" value="1" <?php echo get_option($opt_jfb_show_credit)?'checked="checked"':''?> /> <?php _e("Display a 'Powered By' link in the blog footer (would be appreciated! :))", 'wp-fb-ac')?><br />
                <input type="checkbox" name="<?php echo $opt_jfb_reportstats?>" value="1" <?php echo get_option($opt_jfb_reportstats)?'checked="checked"':''?> /> <?php _e("Periodically report usage stats to the plugin author", 'wp-fb-ac')?>  <?php jfb_output_simple_lightbox("", __("Reports include some very basic server info & a login count, to help me understand how & where the plugin is most used, and how I should focus my development.  No names or e-mail addresses are sent.", 'wp-fb-ac'));?><br />
        
        		<br /><b><?php _e("Debug", 'wp-fb-ac')?>:</b><br />
        		<?php add_option($opt_jfb_email_to, get_bloginfo('admin_email')); ?>
        		<input id="log_mainoption" type="checkbox" name="<?php echo $opt_jfb_email_logs?>" value="1" <?php echo get_option($opt_jfb_email_logs)?'checked="checked"':''?> /> <?php _e("Send event logs to:", 'wp-fb-ac')?> <input type="text" size="40" name="<?php echo $opt_jfb_email_to?>" value="<?php echo get_option($opt_jfb_email_to) ?>" /> <?php jfb_output_simple_lightbox("", __("Event logs show detailed information about the login process, and are useful for debugging various types of issues.  However, note that this option will send you an e-mail every time the login form is submitted - whether it's by a person or an automated spambot probing your site for vulnerabilities.  The latter type of submission is very common and is nothing to worry about, as both Wordpress and this plugin have various types of security to prevent the bots from getting in.  To avoid these spambot-generated messages, you may disable the suboption below.", 'wp-fb-ac'));?><br />
        		<input id="log_suboption" style="margin-left:20px;" type="checkbox" name="<?php echo $opt_jfb_email_logs_missingpost?>" value="1" <?php echo get_option($opt_jfb_email_logs_missingpost)?'checked="checked"':''?> /> ...<?php echo sprintf(__("Include '%s' error events", "wp-fb-ac"), 'Missing POST Data')?><br />
        		<input type="checkbox" name="<?php echo $opt_jfb_disablenonce?>" value="1" <?php echo get_option($opt_jfb_disablenonce)?'checked="checked"':''?> /> <?php _e("Disable nonce security check",'wp-fb-ac')?> (<?php _e("Not recommended", 'wp-fb-ac')?>)<br />
                <input type="checkbox" name="<?php echo $opt_jfb_delay_redir?>" value="1" <?php echo get_option($opt_jfb_delay_redir)?'checked="checked"':''?> /> <?php _e("Delay redirect after login", 'wp-fb-ac')?> (<i><u><?php _e("Not for production sites!", 'wp-fb-ac')?></u></i>)<br />
                <input type="checkbox" name="<?php echo $opt_jfb_fulllogerr?>" value="1" <?php echo get_option($opt_jfb_fulllogerr)?'checked="checked"':''?> /> <?php _e("Show full log on error", 'wp-fb-ac')?> (<i><u><?php _e("Not for production sites!", 'wp-fb-ac')?></u></i>)<br />
                <script type="text/javascript">
                ...
                <div class="submit"><input type="submit" name="Submit" value="<?php _e("Save", 'wp-fb-ac')?>" /></div>
            </form>
    	</div><!-- End Tab -->
        ...
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[3]?>" style="display:<?php echo ($shownTab==3?"block":"none")?>">
            <h3><?php _e("Delete All Plugin Options", 'wp-fb-ac')?></h3>
            <?php _e("The following button will PERMANENTLY delete all of this plugin's options from your Wordpress database, as if it had never been installed.  Use with care!", 'wp-fb-ac')?>
            <form name="formDebugOptions" method="post" action="">
                <input type="hidden" name="remove_all_settings" value="1" />
                <div class="submit"><input type="submit" name="Submit" value="<?php _e("Delete", 'wp-fb-ac')?>" /></div>
            </form>
        </div> <!-- End Tab -->
        
        <div class="<?php echo $allTabsClass ?>" id="<?php echo $tabIds[4]?>" style="display:<?php echo ($shownTab==4?"block":"none")?>">
            <h3><?php _e("Support Information", 'wp-fb-ac')?></h3>
            <div style="width:600px;">
            <?php echo sprintf(__("Before submitting a support request, please make sure to carefully read all the documentation and FAQs on the %s plugin homepage %s.  Every problem that's ever been reported has a solution posted there.", 'wp-fb-ac'), "<a href='$jfb_homepage#faq' target='support'>", "</a>")?><br /><br />                        
            <?php echo sprintf(__("If you do choose to submit a request, please do so via the %s plugin homepage %s, not on Wordpress.org (which I rarely check).  Also, please specifically mention that you've tried it with all other plugins disabled and the default theme (see %s) and include the following information about your Wordpress environment:", 'wp-fb-ac'), "<a href='$jfb_homepage#feedback' target='support'>", "</a>", "<a href='$jfb_homepage#faq100' target='faq100'>FAQ100</a>")?><br /><br />
            </div>
            <textarea readonly="readonly" onclick="this.focus();this.select();" title="<?php _e("To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac)", "wp-fb-ac")?>" style="width:800px; height:350px; font-family: Menlo, Monaco, monospace;">
...
function jfb_output_simple_lightbox($deprecated, $contents)
{
    ...
    ?>
    <a class="wpfb-moreinfo-lnk" href="javascript:void(0)" title="<?php echo $contents?>" onclick="document.getElementById('wpfb-moreinfo-bg<?php echo $jfbLbID?>').style.display='block'; document.getElementById('wpfb-moreinfo-box<?php echo $jfbLbID?>').style.display='block';">(<?php _e('Click for more info','wp-fb-ac'); ?>)</a>
    ...
    <div class="wpfb-moreinfo-box" id="wpfb-moreinfo-box<?php echo $jfbLbID?>">
      <a href="javascript:void(0)" onclick="document.getElementById('wpfb-moreinfo-bg<?php echo $jfbLbID?>').style.display='none'; document.getElementById('wpfb-moreinfo-box<?php echo $jfbLbID?>').style.display='none';"><?php _e("Close", 'wp-fb-ac')?></a><br />
...
function jfb_output_premium_panel_tease()
{
    ...
    <form name="formPremOptions" method="post" action="">
        ...
        <b><?php _e('Avatar Caching', 'wp-fb-ac')?>:</b><br />  
        ...
        <?php _e('Cache Facebook avatars locally', 'wp-fb-ac')?> (<?php _e('fullsize', 'wp-fb-ac')?>) <?php jfb_output_simple_lightbox("", __("Because most themes only utilize thumbnail-sized avatars, caching full-sized images is often unnecessary.  If you're not actually using full-sized avatars I recommend disabling this option, as doing so will speed up logins and save space on your server (there's a small per-login performance cost to copying the files locally).", 'wp-fb-ac'))?><br />
        ...
        <input <?php disableatt() ?> type="checkbox" name="<?php echo $opt_jfbp_requirerealmail?>" value="1" <?php echo get_option($opt_jfbp_requirerealmail)?'checked="checked"':''?> /> <?php _e("Enforce access to user's email", 'wp-fb-ac')?>
        <?php jfb_output_simple_lightbox("", __("The basic option to request user emails will prompt your visitors, but they can still hide their true addresses by using a Facebook proxy (click 'change' in the permissions dialog, and select 'xxx@proxymail.facebook.com').  This option performs a secondary check to enforce that they allow access to their REAL e-mail.  Note that the check requires several extra queries to Facebook's servers, so it could result in a slightly longer delay before the login initiates.", 'wp-fb-ac'))?><br />
        <small><i><?php _e('*IMPORTANT: This feature only works with apps created before August 7th, 2014', 'wp-fb-ac')?></i></small>
        <?php jfb_output_simple_lightbox("", __("When Facebook released API v2.1 on August 7th, 2014, they removed several functions required by this feature.  They also prevent new applications from targeting older versions of the API (see <a href='https://developers.facebook.com/docs/apps/versions' target='dumbfb'>here</a>, under 'Can my app make calls to versions older than the current version?').<br/><br/>What this means is that the feature needs to be completely rewritten to work with newer sites.  For the time being, I leave it here as a convenience for older customers who were already using it.  For new users, enabling this option will have no effect until I can figure out a workaround for Facebook's latest round of breaking changes.", 'wp-fb-ac'))?><br /><br/>
        
        <b><?php _e('Wordbooker Avatar Integration', 'wp-fb-ac')?>:</b><br />
		...
		<input <?php disableatt() ?> type="radio" name="<?php echo $opt_jfbp_restrict_reg; ?>" value="4" <?php echo (get_option($opt_jfbp_restrict_reg)==4?"checked='checked'":"")?>><?php _e('Membership - Only autoregister members of Facebook group id:', 'wp-fb-ac')?> <input <?php disableatt() ?> type="text" size="15" name="<?php echo $opt_jfbp_restrict_reg_gid?>" value="<?php echo get_option($opt_jfbp_restrict_reg_gid); ?>" /> <?php jfb_output_simple_lightbox("", sprintf(__("To find a group's id, view its URL.  It will be something like '%s'.  In this example, the group id would be 12345678.", 'wp-fb-ac'), "http://www.facebook.com/group.php?gid=12345678"))?><br />
		...
		<small>&bull; <?php _e('Facebook fields marked with an asterisk (i.e. Birthday*) require the user to approve extra permissions during login.', 'wp-fb-ac')?></small><br />
