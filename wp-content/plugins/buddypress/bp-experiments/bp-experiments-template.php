...
function bp_experiment_member_count() {
	...
	function bp_get_experiment_member_count() {
		...
		if ( 1 == (int) $experiments_template->experiment->total_member_count )
			return apply_filters( 'bp_get_experiment_member_count', sprintf( __( '<span data-icon="c" class="icon-small"></span> %s ', 'buddypress' ), bp_core_number_format( $experiments_template->experiment->total_member_count ) ) );
		else
			return apply_filters( 'bp_get_experiment_member_count', sprintf( __( '<span data-icon="c" class="icon-small"></span> %s ', 'buddypress' ), bp_core_number_format( $experiments_template->experiment->total_member_count ) ) );
	}
...
function bp_experiment_admin_tabs( $experiment = false ) {
	...
	<?php if ( !(int)bp_get_option( 'bp-disable-avatar-uploads' ) ) : ?>
		...
		<li<?php if ( 'experiment-avatar'   == $current_tab ) : ?> class="current"<?php endif; ?>><a href="<?php echo trailingslashit( bp_get_experiment_permalink( $experiment ) . 'admin/experiment-avatar' ) ?>"><?php _e( 'Logo', 'buddypress' ); ?></a></li>
	...
	function bp_get_experiment_join_button( $experiment = false ) {
		...
		if ( isset( $experiment->is_member ) && $experiment->is_member ) {
			...
			$button = array(
				...
				'link_text'         => '<i data-icon="s" class="icon"></i><l style="vertical-align:top; margin-top:20px;font-size:2em"> Leave</l>',
			...
			switch ( $experiment->status ) {
				...
				case 'public':
					$button = array(
						...
						'link_text'         => '<i data-icon="j" class="icon"></i> <l style="vertical-align:top; margin-top:20px;font-size:2em"> Join</l>',
				...
				case 'private' :
					...
					if ( $experiment->is_invited ) {
						$button = array(
							...
							'link_text'         =>'<i data-icon="j" class="icon"></i> <l style="vertical-align:top; margin-top:20px;font-size:2em">Accept Invitation</l>',
					...
					// show a "Request Sent" button
						...
						$button = array(
							...
							'link_text'         =>  '<i data-icon="s" class="icon"></i> <l style="vertical-align:top; margin-top:20px;font-size:2em">Asked to join</l>',
					...
					// show a "Request Membership" button
						...
						$button = array(
							...
							'link_text'         => '<i data-icon="j" class="icon"></i> <l style="vertical-align:top; margin-top:20px;font-size:2em">Ask to join</l>',
...
function bp_new_experiment_invite_friend_list() {
	...
	function bp_get_new_experiment_invite_friend_list( $args = '' ) {
		...
		//echo "experiment_id changed =". $experiment_id ;

		//if ( $friends = friends_get_friends_invite_list( bp_loggedin_user_id(), $experiment_id ) ) {
		if ( $friends = friends_get_friends_invite_list_experiment( bp_loggedin_user_id(), $experiment_id ) ) {
			//echo "friends to invite";
			...
			for ( $i = 0, $count = count( $friends ); $i < $count; ++$i ) {
				...
				$items[] = $friends[$i]['avatar'] . '<' . $separator . '><input' . $checked . ' type="checkbox" tabindex="-1" name="friends[]" id="f-' . $friends[$i]['id'] . '" value="' . esc_attr( $friends[$i]['id'] ) . '" /> ' . $friends[$i]['full_name'] . '</' . $separator . '>';
			}
		}//end if
...
function bp_directory_experiments_search_form() {
	...
	$search_form_html = '<form action="" method="get" id="search-experiments-form">
		<label><input type="text" name="s" id="experiments_search" placeholder="Search" /></label>
		<button type="submit" id="experiments_search_submit" name="experiments_search_submit"><span class="glyphicon glyphicon-search"></span></button>
	</form>';
