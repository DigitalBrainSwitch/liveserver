 ...
 * or when the Experiments component is disabled
 */
...
<script src='http://www.myliferocket.com/wp-content/plugins/buddypress/bp-templates/bp-legacy/js/bootstrap-switch.min.js'></script>
<link rel="stylesheet" type="text/css" href="http://www.myliferocket.com/clockpicker/bootstrap-clockpicker.min.css">
<script type="text/javascript" src="http://www.myliferocket.com/clockpicker/bootstrap-clockpicker.min.js"></script>
<h1>Reminders</h1>
    <p>Set-up email reminders so you don't forget to report on the experiment.</p>
    <div class='col-md-4'>Time</div>
    <div class='col-md-4'>Frequency</div>
    <div class='col-md-4'>Enabled</div>
<div id='timepickers' style='margin-top:10px'>
</div><button id='newNotification'>+</button></br></br>
<button id='saveNotifications'><i data-icon="l" class="icon"></i></button> <a href=''>Cancel</a>
<script>
var newCount = 0;
$( document ).ready(function() {
    ...
    $.ajax({
      async:false,
      type: "POST",
      //url: "http://www.myliferocket.com/wp-includes/getNotifications.php",
      url: "http://www.myliferocket.com/ajaxfiles/getNotifications.php",
      data:{
...
function createTimepicker(time, id, enabled, type){
    ...
    var id = id;
    $('#timepickers').append("<div class='row notification'><div id='"+id+"'><div class='col-md-4'><input class='timepicker' id='timepicker"+id+"' style='text-align:center' value='"+time+"' type='text'></input></div></div></div>");
...
function saveNotification(id, enabled, time, type){
    ...
    $.ajax({
          async:false,
          type: "POST",
          url: "http://www.myliferocket.com/ajaxfiles/saveNotifications.php",
          //url: "http://www.myliferocket.com/wp-includes/saveNotifications.php",
          data:{
             id:  id, 
             experiment_id:  <?php echo bp_get_current_experiment_id()?>, 
             user_id: <?php echo get_current_user_id(); ?>,
...
class experiment_Extension_Example_3 extends BP_experiment_Extension {
    /**
     * Here you can see more customization of the config options
     */
    function __construct() {
...
<h1>Comments</h1>
    <p>Post your comments on the experiment.</p>
    <div class='col-md-4'>Time</div>
    <div class='col-md-4'>Frequency</div>
    <div class='col-md-4'>Enabled</div>

<?php
    }//end if(bp_experiment_is_member())
    else{
        echo "join/login.";
    }
}
    ...
    function settings_screen( $experiment_id ) {
        $setting = experiments_get_experimentmeta( $experiment_id, 'experiment_extension_example_3_setting' );
 
        ?>
        Save your plugin setting here: <input type="text" name="experiment_extension_example_3_setting" value="<?php echo esc_attr( $setting ) ?>" />
        <?php
    }
 
    function settings_screen_save( $experiment_id ) {
        $setting = isset( $_POST['experiment_extension_example_3_setting'] ) ? $_POST['experiment_extension_example_3_setting'] : '';
        experiments_update_experimentmeta( $experiment_id, 'experiment_extension_example_3_setting', $setting );
    }
...
bp_register_experiment_extension( 'experiment_Extension_Example_3' );
