...
function bp_core_add_page_mappings( $components, $existing = 'keep' ) {
	...
	$page_titles = array(
        ...
        'experiments'   => _x( 'Experiments', 'Page title for the Experiments directory.', 'buddypress' ),
...
function bp_core_action_search_site( $slug = '' ) {
	...
	if ( empty( $slug ) ) {
		switch ( $search_which ) {
			...
			case 'experiments':
				$slug = bp_is_active( 'experiments' ) ? bp_get_experiments_root_slug() : '';
				break;
...
echo '<li class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children dropdown menu-item-27" id="top-notification">';
echo '<a class="dropdown-toggle" data-toggle="dropdown" data-target="#" href="'. $bp->loggedin_user->domain . 'notifications"><span title="Notifications "style="font-size:1.3em;" data-icon="p" class="icon icon-small"></span> ';
...
if ( $notifications ) {
    ...
    for ( $i = 0; $i < count($notifications); $i++ ) {
        ...
        <li<?php echo $alt ?> class='menu-item'><?php echo $notifications[$i] ?></li>

        <?php $counter++;
    }
} else { ?>
	
    <li class='alt'><a href="<?php echo $bp->loggedin_user->domain ?>notifications/read">You have no unread notifications.</a></li>
