...
function bp_core_install( $active_components = false ) {
	...
	if ( !empty( $active_components['experiments'] ) )
		bp_core_install_experiments();
...
function bp_core_install_experiments() {
        $sql             = array();
        $charset_collate = bp_core_set_charset();
        $bp_prefix       = bp_core_get_table_prefix();
        
        $sql[] = "CREATE TABLE {$bp_prefix}bp_experiments (
        id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
        creator_id bigint(20) NOT NULL,
        name varchar(100) NOT NULL,
        slug varchar(200) NOT NULL,
        ...
        $sql[] = "CREATE TABLE {$bp_prefix}bp_experiments_members (
        id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
        ...
        $sql[] = "CREATE TABLE {$bp_prefix}bp_experiments_experimentmeta (
        id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
        experiment_id bigint(20) NOT NULL,
        meta_key varchar(255) DEFAULT NULL,
        meta_value longtext DEFAULT NULL,
        ...
        $sql[] = "CREATE TABLE {$bp_prefix}bp_experiments_variables (
        id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
        experiment_id bigint(20) NOT NULL,
        name varchar(100) NOT NULL,
        ...
        $sql[] = " CREATE TABLE {$bp_prefix}bp_experiments_report (
        id bigint(20) NOT NULL AUTO_INCREMENT PRIMARY KEY,
        experiment_id bigint(20) NOT NULL,
        user_id bigint(20) NOT NULL,
        ...
        $sql[] = " CREATE TABLE {$bp_prefix}bp_experiments_notifications (
...
function bp_core_install_extended_profiles() {
	...
	// These values should only be updated if they are not already present
	if ( ! bp_get_option( 'bp-xprofile-base-experiment-name' ) ) {
		bp_update_option( 'bp-xprofile-base-experiment-name', _x( 'General', 'First field-experiment name', 'buddypress' ) );
	}
    ...
    $sql[] = "CREATE TABLE {$bp_prefix}bp_xprofile_experiments (
    id bigint(20) unsigned NOT NULL AUTO_INCREMENT PRIMARY KEY,
    name varchar(150) NOT NULL,
    description mediumtext NOT NULL,
    experiment_order bigint(20) NOT NULL DEFAULT '0',
    can_delete tinyint(1) NOT NULL,
    KEY can_delete (can_delete)
    ) {$charset_collate};";
    ...
    // Insert the default experiment and fields
	$insert_sql = array();
    
	if ( !$wpdb->get_var( "SELECT id FROM {$bp_prefix}bp_xprofile_experiments WHERE id = 1" ) )
		$insert_sql[] = "INSERT INTO {$bp_prefix}bp_xprofile_experiments ( name, description, can_delete ) VALUES ( " . $wpdb->prepare( '%s', stripslashes( bp_get_option( 'bp-xprofile-base-experiment-name' ) ) ) . ", '', 0 );";
    
	if ( !$wpdb->get_var( "SELECT id FROM {$bp_prefix}bp_xprofile_fields WHERE id = 1" ) )
		$insert_sql[] = "INSERT INTO {$bp_prefix}bp_xprofile_fields ( experiment_id, parent_id, type, name, description, is_required, can_delete ) VALUES ( 1, 0, 'textbox', " . $wpdb->prepare( '%s', stripslashes( bp_get_option( 'bp-xprofile-fullname-field-name' ) ) ) . ", '', 1, 0 );";
