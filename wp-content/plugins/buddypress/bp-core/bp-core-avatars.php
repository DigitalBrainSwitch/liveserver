...
function bp_core_fetch_avatar( $args = '' ) {
	...
	if ( false !== strpos( $alt, '%s' ) || false !== strpos( $alt, '%1$s' ) ) {
		...
		switch ( $object ) {
			...
			case 'experiment' :
				$item_name = bp_get_experiment_name( experiments_get_experiment( array( 'experiment_id' => $item_id ) ) );
				break;
	...
	$legacy_experiment_avatar_name = ( 'full' == $type ) ? '-experimentavatar-full' : '-experimentavatar-thumb';
		...
		if ( $av_dir = opendir( $avatar_folder_dir ) ) {
			...
			if ( 0 < count( $avatar_files ) ) {
				...
				if ( !isset( $avatar_url ) ) {
					...
					// Legacy group/experiment avatar check
					if ( !isset( $avatar_url ) ) {
						foreach( $avatar_files as $key => $value ) {
                            ...
                            if ( strpos ( $value, $legacy_experiment_avatar_name )!== false ) {
								$avatar_url = $avatar_folder_url . '/' . $avatar_files[$key];
							}
	...
	if ( ! apply_filters( 'bp_core_fetch_avatar_no_grav', $no_grav ) ) {
		...
		if ( empty( $email ) ) {
			...
			} else if ( 'group' == $object || 'experiment' == $object || 'blog' == $object ) {
	...
	if ( true === $html ) {
		return apply_filters( 'bp_core_fetch_avatar', '<img src="' . 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg' . '" class="' . esc_attr( $class ) . '"' . $css_id . $html_width . $html_height . $html_alt . $title . ' />', $params, $item_id, $avatar_dir, $css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
	} else {
		return apply_filters( 'bp_core_fetch_avatar_url', 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg', $params );
	}
}
...
function bp_core_delete_existing_avatar( $args = '' ) {
	...
	if ( empty( $item_id ) ) {
        ...
        else if ( 'experiment' == $object )
			$item_id = buddypress()->experiments->current_experiment->id;
		else if ( 'blog' == $object )
			$item_id = $current_blog->id;
...
function bp_core_avatar_default( $type = 'gravatar' ) {
	...
	define ( 'BP_AVATAR_DEFAULT', 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg' );
	define ( 'BP_AVATAR_DEFAULT_THUMB', 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg' );
	if ( defined( 'BP_AVATAR_DEFAULT' ) ) {
		$avatar = BP_AVATAR_DEFAULT;

	// Use the local default image
	} else if ( 'local' === $type ) {
		$avatar = 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg';
	...
	return apply_filters( 'bp_core_avatar_default', 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg' );
}
...
function bp_core_avatar_default_thumb( $type = 'gravatar' ) {
	...
	return apply_filters( 'bp_core_avatar_thumb', 'http://www.myliferocket.com/wp-content/themes/ward-pro/library/images/default.jpg');
}