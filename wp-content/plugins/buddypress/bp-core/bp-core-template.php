...
//		$list_type = bp_is_group() ? 'groups' : 'personal';
		...
		if($subnav_item['css_id']=="home"){
			...
			$subnav_item['title'] = 'Report';
		}if($subnav_item['css_id']=="members"){
			$subnav_item['name'] = "<div data-icon='c' class='icon'></div>";
			$subnav_item['title'] = 'Members';
		}if($subnav_item['css_id']=="invite"){
			$subnav_item['name'] = '<div data-icon="b" class="icon"></div>';
			$subnav_item['title'] = 'Invite Friends';
		}if($subnav_item['css_id']=="admin"){
			$subnav_item['name'] = '<div data-icon="d" class="icon"></div>';
			$subnav_item['title'] = 'Settings';
		}if($subnav_item['css_id']=="nav-reminders"){
			$subnav_item['name'] = '<div data-icon="m" class="icon"></div>';
			$subnav_item['title'] = 'Reminders';
		}if($subnav_item['css_id']=="activity"){
			$subnav_item['name'] = '<div data-icon="x" class="icon"></div>';
			$subnav_item['title'] = 'Activity';
		}
		
		// echo out the final list item
		echo apply_filters( 'bp_get_options_nav_' . $subnav_item['css_id'], '<li id="' . $subnav_item['css_id'] . '-' . $list_type . '-li" ' . $selected . '><a title=' . $subnav_item['title'] . ' id="' . $subnav_item['css_id'] . '" href="' . $subnav_item['link'] . '">' . $subnav_item['name'] . '</a></li>', $subnav_item );
	}
}
...
function bp_search_form_type_select() {
    ...
    if ( bp_is_active( 'experiments' ) )
		$options['experiments']  = __( 'Experiments',  'buddypress' );
...
/**
 * Check whether the current page is part of the Experiments component.
 *
 * @return bool True if the current page is part of the Experiments component.
 */
function bp_is_experiments_component() {
	if ( bp_is_current_component( 'experiments' ) )
		return true;
...
/**
 * Is the current page a user's Experiments activity stream?
 *
 * Eg http://example.com/members/joe/experiments/
 *
 * @return True if the current page is a user's Experiments activity stream.
 */
function bp_is_user_experiments_activity() {
    
	if ( !bp_is_active( 'experiments' ) )
		return false;
    
	$slug = bp_get_experiments_slug();
...
/**
 * Is the current page part of a user's Experiments page?
 *
 * Eg http://example.com/members/joe/experiments/ (or a subpage thereof).
 *
 * @return bool True if the current page is a user's Experiments page.
 */
function bp_is_user_experiments() {
	if ( bp_is_user() && bp_is_experiments_component() )
		return true;
...
/**
 * Is the current page the experiments directory ?
 *
 * @since BuddyPress (2.0.0)
 *
 * @return True if the current page is the experiments directory.
 */
function bp_is_experiments_directory() {
	if ( bp_is_experiments_component() && ! bp_current_action() && ! bp_current_item() )
		return true;
...
/**
 * Does the current page belong to a single experiment?
 *
 * Will return true for any subpage of a single experiment.
 *
 * @return bool True if the current page is part of a single experiment.
 */
function bp_is_experiment() {
	global $bp;
    
	if ( bp_is_experiments_component() && isset( $bp->experiments->current_experiment ) && $bp->experiments->current_experiment )
		return true;
...
/**
 * Is the current page a single experiment's home page?
 *
 * URL will vary depending on which experiment tab is set to be the "home". By
 * default, it's the experiment's recent activity.
 *
 * @return bool True if the current page is a single experiment's home page.
 */
function bp_is_experiment_home() {
	if ( bp_is_single_item() && bp_is_experiments_component() && ( !bp_current_action() || bp_is_current_action( 'home' ) ) )
		return true;
...
/**
 * Is the current page part of the experiment creation process?
 *
 * @return bool True if the current page is part of the experiment creation process.
 */
function bp_is_experiment_create() {
...
/**
 * Is the current page part of a single experiment's admin screens?
 *
 * Eg http://example.com/experiments/myexperiment/admin/settings/.
 *
 * @return bool True if the current page is part of a single experiment's admin.
 */
function bp_is_experiment_admin_page() {
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'admin' ) )
		return true;
...
/**
 * Is the current page a experiment's forum page?
 *
 * Only applies to legacy bbPress forums.
 *
 * @return bool True if the current page is a experiment forum page.
 */
function bp_is_experiment_forum() {
	$retval = false;
    
	// At a forum URL
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'forum' ) ) {
		$retval = true;
...
/**
 * Is the current page a experiment's activity page?
 *
 * @return True if the current page is a experiment's activity page.
 */
function bp_is_experiment_activity() {
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'activity' ) )
		return true;
...
/**
 * Is the current page a experiment forum topic?
 *
 * Only applies to legacy bbPress (1.x) forums.
 *
 * @return bool True if the current page is part of a experiment forum topic.
 */
function bp_is_experiment_forum_topic() {
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'forum' ) && bp_is_action_variable( 'topic', 0 ) )
		return true;
...
/**
 * Is the current page a experiment forum topic edit page?
 *
 * Only applies to legacy bbPress (1.x) forums.
 *
 * @return bool True if the current page is part of a experiment forum topic edit page.
 */
function bp_is_experiment_forum_topic_edit() {
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'forum' ) && bp_is_action_variable( 'topic', 0 ) && bp_is_action_variable( 'edit', 2 ) )
		return true;
...
/**
 * Is the current page a experiment's Members page?
 *
 * Eg http://example.com/experiments/myexperiment/members/.
 *
 * @return bool True if the current page is part of a experiment's Members page.
 */
function bp_is_experiment_members() {
	if ( bp_is_single_item() && bp_is_experiments_component() && bp_is_current_action( 'members' ) )
		return true;
...
/**
 * Is the current page a experiment's Invites page?
 *
 * Eg http://example.com/experiments/myexperiment/send-invites/.
 *
 * @return bool True if the current page is a experiment's Send Invites page.
 */
function bp_is_experiment_invites() {
	if ( bp_is_experiments_component() && bp_is_current_action( 'send-invites' ) )
		return true;
...
/**
 * Is the current page a experiment's Request Membership page?
 *
 * Eg http://example.com/experiments/myexperiment/request-membership/.
 *
 * @return bool True if the current page is a experiment's Request Membership page.
 */
function bp_is_experiment_membership_request() {
	if ( bp_is_experiments_component() && bp_is_current_action( 'request-membership' ) )
		return true;
...
/**
 * Is the current page a leave experiment attempt?
 *
 * @return bool True if the current page is a Leave Experiment attempt.
 */
function bp_is_experiment_leave() {
    
	if ( bp_is_experiments_component() && bp_is_single_item() && bp_is_current_action( 'leave-experiment' ) )
		return true;
...
/**
 * Is the current page part of a single experiment?
 *
 * Not currently used by BuddyPress.
 *
 * @todo How is this functionally different from bp_is_experiment()?
 *
 * @return bool True if the current page is part of a single experiment.
 */
function bp_is_experiment_single() {
...
function bp_the_body_class() {
	...
	function bp_get_the_body_class( $wp_classes = array(), $custom_classes = false ) {
		...
		if ( !bp_is_directory() ) :
            ...
            if ( bp_is_user_experiments() )
                $bp_classes[] = 'my-experiments';
        ...
        if ( bp_is_user_experiments_activity() )
			$bp_classes[] = 'experiments-activity';
		...
		if ( bp_is_experiment_leave() )
			$bp_classes[] = 'leave-experiment';
        
		if ( bp_is_experiment_invites() )
			$bp_classes[] = 'experiment-invites';
        
		if ( bp_is_experiment_members() )
			$bp_classes[] = 'experiment-members';
        
		if ( bp_is_experiment_forum_topic() )
			$bp_classes[] = 'experiment-forum-topic';
        
		if ( bp_is_experiment_forum_topic_edit() )
			$bp_classes[] = 'experiment-forum-topic-edit';
        
		if ( bp_is_experiment_forum() )
			$bp_classes[] = 'experiment-forum';
        
		if ( bp_is_experiment_admin_page() ) {
			$bp_classes[] = 'experiment-admin';
			$bp_classes[] = bp_get_experiment_current_admin_tab();
		}
        
		if ( bp_is_experiment_create() ) {
			$bp_classes[] = 'experiment-create';
			$bp_classes[] = bp_get_experiments_current_create_step();
		}
        
		if ( bp_is_experiment_home() )
			$bp_classes[] = 'experiment-home';
