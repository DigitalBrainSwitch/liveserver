...
/**
 * Get a list of friends that a user can invite into this experiment.
 *
 * Excludes friends that are already in the experiment, and banned friends if the
 * user is not a experiment admin.
 *
 * @since BuddyPress (1.0.0)
 *
 * @param int $user_id User ID whose friends to see can be invited. Default:
 *        ID of the logged-in user.
 * @param int $experiment_id Experiment to check possible invitations against.
 * @return mixed False if no friends, array of users if friends.
 */
function friends_get_friends_invite_list_experiment( $user_id = 0, $experiment_id = 0 ) {

	// Default to logged in user id
	if ( empty( $user_id ) )
		$user_id = bp_loggedin_user_id();

	// Only experiment admins can invited previously banned users
	//$user_is_admin = (bool) experiments_is_user_admin( $user_id, $experiment_id );
	$user_is_admin = true;
	...
	if ( bp_has_members( $args ) ) {
		 ...
		 *
		 * Exclude friends that:
		 *     1. are already members of the experiment
		 *     2. are banned from this experiment if the current user is also not a
		 *        experiment admin.
		 */
		while ( bp_members() ) :
			...
			// Skip friend if already in the group
			if ( experiments_is_user_member( $friend_user_id, $experiment_id ) )
				continue;

			// Skip friend if not experiment admin and user banned from experiment
			if ( ( false === $user_is_admin ) && experiments_is_user_banned( $friend_user_id, $experiment_id ) )
				continue;
	...
	// Allow friends to be filtered
	return apply_filters( 'bp_friends_get_invite_list_experiment', $friends, $user_id, $experiment_id );
}

/**
 * Get a count of a user's friends who can be invited to a given experiment.
 *
 * Users can invite any of their friends except:
 *
 * - users who are already in the experiment
 * - users who have a pending invite to the experiment
 * - users who have been banned from the experiment
 *
 * @param int $user_id ID of the user whose friends are being counted.
 * @param int $experiment_id ID of the experiment friends are being invited to.
 * @return int $invitable_count Eligible friend count.
 */
function friends_count_invitable_friends_experiment( $user_id, $experiment_id ) {
	return BP_Friends_Friendship::get_invitable_friend_count_experiment( $user_id, $experiment_id );
}
