...
class BP_Friends_Friendship {
	 ...
	 * @return int $invitable_count Eligible friend count.
	 */
	public static function get_invitable_friend_count_experiment( $user_id, $experiment_id ) {

		// Setup some data we'll use below
		//$is_experiment_admin  = BP_Experiments_Member::check_is_admin( $user_id, $experiment_id );
		$is_experiment_admin  = true;
		$friend_ids      = BP_Friends_Friendship::get_friend_user_ids( $user_id );
		$invitable_count = 0;
		...
		for ( $i = 0, $count = count( $friend_ids ); $i < $count; ++$i ) {
			...
			// If already a member, they cannot be invited again
			if ( BP_Experiments_Member::check_is_member( (int) $friend_ids[$i], $experiment_id ) )
				continue;

			// If user already has invite, they cannot be added
			if ( BP_Experiments_Member::check_has_invite( (int) $friend_ids[$i], $experiment_id )  )
				continue;

			// If user is not experiment admin and friend is banned, they cannot be invited
			if ( ( false === $is_experiment_admin ) && BP_Experiments_Member::check_is_banned( (int) $friend_ids[$i], $experiment_id ) )
				continue;
