 ...
 *
 * This is responsible for loading a experiment of activity items and displaying them.
 *
 ...
 * @uses experiments_is_user_member()
 ...
 * @uses experiments_get_user_experiments()
 ...
 *               current user
 *             - 'experiments' retrieves items associated with the experiments to which
 ...
 *           be 'experiments' when visiting http://example.com/members/joe/activity/experiments/.
 ...
 *           Friends, My experiments, etc), 'user_id' defaults to the ID of the
 ...
 *           For example, 'experiments' will limit results to those that are
 *           associated with the BP experiments component. Accepts a single
 *           component string, or an array of multiple components. Defaults to
 *           'experiments' when viewing the page of a single experiment, the My experiments
 *           activity filter, or the Activity > experiments filter of a user profile.
 *           Otherwise defaults to false.
 ...
 *           'new_blog_post', 'created_experiment'). Accepts a single type string,
 ...
 *           types; for example, in the case of 'created_experiment', 'primary_id'
 *           is the ID of the experiment. Accepts a single ID, or an array of
 *           multiple IDs. When viewing a single experiment, defaults to the current
 *           experiment ID. When viewing a user's experiments stream page, defaults to
 *           the IDs of the user's experiments. Otherwise defaults to false.
 ...
 *             - User is viewing the activity stream of a non-public experiment of
 *               which he is a member
 *     @type bool $show_hidden Normally defaults to false, except when:
 *             - a user is viewing his own activity stream
 *             - a user is viewing the activity stream of a non-public experiment of
 *               which he is a member
...
function bp_has_activities( $args = '' ) {
	...
	if ( !empty( $bp->experiments->current_experiment ) ) {
		$object = $bp->experiments->id;
		$primary_id = $bp->experiments->current_experiment->id;

		if ( ( 'public' != $bp->experiments->current_experiment->status ) && ( experiments_is_user_member( bp_loggedin_user_id(), $bp->experiments->current_experiment->id ) || bp_current_user_can( 'bp_moderate' ) ) )
			$show_hidden = true;
	}
	...
	// Support for permalinks on single item pages: /experiments/my-experiment/activity/124/
	...
	$defaults = array(
		...
		// Scope - pre-built activity filters for a user (friends/experiments/favorites/mentions)
		'scope'             => $scope,

		// Filtering
		'user_id'           => $user_id,     // user_id to filter on
		'object'            => $object,      // object to filter on e.g. experiments, profile, status, friends
		'action'            => false,        // action to filter on e.g. activity_update, new_forum_post, profile_updated
		'primary_id'        => $primary_id,  // object ID to filter on e.g. a experiment_id or forum_id or blog_id etc.
	...
	if ( 'just-me' == $scope || 'friends' == $scope || 'experiments' == $scope || 'favorites' == $scope || 'mentions' == $scope ) {
		if ( 'just-me' == $scope )
			...
			switch ( $scope ) {
				...
				case 'experiments':
					if ( bp_is_active( 'experiments' ) ) {
						$experiments = experiments_get_user_experiments( $user_id );
						if ( empty( $experiments['experiments'] ) )
							return false;

						$object = $bp->experiments->id;
						$primary_id = implode( ',', (array) $experiments['experiments'] );
	...
	function bp_get_activity_secondary_avatar( $args = '' ) {
		...
		switch ( $activities_template->activity->component ) {
			case 'experiments' :
				...
				if ( empty( $alt ) ) {
					$alt = __( 'experiment logo', 'buddypress' );

					if ( ! empty( $name ) ) {
						$alt = sprintf( __( 'experiment logo of %s', 'buddypress' ), $name );
					}
				}
...
function bp_activities_member_rss_link() { echo bp_get_member_activity_feed_link(); }
	...
	function bp_get_member_activity_feed_link() {
		...
		elseif ( bp_is_active( 'experiments'  ) && bp_is_current_action( bp_get_experiments_slug()  ) )
			$link = bp_displayed_user_domain() . bp_get_activity_slug() . '/' . bp_get_experiments_slug() . '/feed/';
