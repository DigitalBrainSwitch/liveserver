...
/**
 * Load the 'My Experiments' activity page.
 *
 * @since BuddyPress (1.2)
 *
 * @uses bp_is_active()
 * @uses bp_update_is_item_admin()
 * @uses bp_current_user_can()
 * @uses do_action() To call the 'bp_activity_screen_experiments' hook
 * @uses bp_core_load_template()
 * @uses apply_filters() To call the 'bp_activity_template_groups_activity' hook
 */
 
function bp_activity_screen_experiments() {
    
    if ( !bp_is_active( 'experiments' ) )
            return false;
        
    bp_update_is_item_admin( bp_current_user_can( 'bp_moderate' ), 'activity' );
    do_action( 'bp_activity_screen_experiments' );
    bp_core_load_template( apply_filters( 'bp_activity_template_experiments_activity', 'members/single/home' ) );
}
...
function bp_activity_screen_single_activity_permalink() {
	...
	// If activity is from a experiment, do an extra cap check
	if ( isset( $bp->experiments->id ) && $activity->component == $bp->experiments->id ) 
	{

		// Activity is from an experiment, but experiments is currently disabled
		if ( !bp_is_active( 'experiments') ) {
			bp_do_404();
			return;
		}

		// Check to see if the experiment is not public, if so, check the
		// user has access to see this activity
		if ( $experiment = experiments_get_experiment( array( 'experiment_id' => $activity->item_id ) ) ) {

			// Experiment is not public
			if ( 'public' != $experiment->status ) {

				// User is not a member of experiment
				if ( !experiments_is_user_member( bp_loggedin_user_id(), $experiment->id ) ) {
					$has_access = false;
				}
			}
		}
	}//end if ( isset( $bp->experiments->id ) && $activity->component == $bp->experiments->id ) 	
