 ...
 * @uses experiments_get_experiment()
 * @uses bp_get_experiment_permalink()
...
function bp_activity_action_permalink_router() {
    ...
    // Redirect based on the type of activity
	} else if (bp_is_active( 'experiments' ) && $activity->component == buddypress()->experiments->id ) {
		...
		if ( ! empty( $activity->user_id ) ) {
			...
			// Set redirect to experiment activity stream
			if ( $experiment = experiments_get_experiment( array( 'experiment_id' => $activity->item_id ) ) ) {
				$redirect = bp_get_experiment_permalink( $experiment ) . bp_get_activity_slug() . '/' . $activity->id . '/';
			}
		}
...
function bp_activity_action_post_update() {
    ...
    // Post to experiments object
	} else if ( 'experiments' == $object && bp_is_active( 'experiments' ) ) {
		if ( (int) $item_id ) {
			$activity_id = experiments_post_update( array( 'content' => $content, 'experiment_id' => $item_id ) );
		}
...
function bp_activity_action_sitewide_feed() {
	...
	if ( ! bp_is_activity_component() || ! bp_is_current_action( 'feed' ) || bp_is_user() || ! empty( $bp->groups->current_group) || ! empty( $bp->experiments->current_experiment)   )
		return false;
...
/**
 * Load the activity feed for a user's experiments.
 *
 * @since BuddyPress (1.2)
 *
 * @uses bp_is_active()
 ...
 * @return bool False on failure.
 */
function bp_activity_action_my_experiments_feed() {
if ( ! bp_is_active( 'experiments' ) || ! bp_is_user_activity() || ! bp_is_current_action( bp_get_experiments_slug() ) || ! bp_is_action_variable( 'feed', 0 ) ) {
return false;
}

// get displayed user's experiment IDs
$experiments    = experiments_get_user_experiments();
$experiment_ids = implode( ',', $experiments['experiments'] );

// setup the feed
buddypress()->activity->feed = new BP_Activity_Feed( array(
                                                           'id'            => 'myexperiments',
                                                           
                                                           /* translators: Member experiments activity RSS title - "[Site Name] | [User Display Name] | Experiments Activity" */
                                                           'title'         => sprintf( __( '%1$s | %2$s | Experiment Activity', 'buddypress' ), bp_get_site_name(), bp_get_displayed_user_fullname() ),
                                                           
                                                           'link'          => trailingslashit( bp_displayed_user_domain() . bp_get_activity_slug() . '/' . bp_get_experiments_slug() ),
                                                           'description'   => sprintf( __( "Public experiment activity feed of which %s is a member of.", 'buddypress' ), bp_get_displayed_user_fullname() ),
                                                           'activity_args' => array(
                                                                                    'object'           => buddypress()->experiments->id,
                                                                                    'primary_id'       => $experiment_ids,
                                                                                    'display_comments' => 'threaded'
                                                                                    )
                                                           ) );
}
add_action( 'bp_actions', 'bp_activity_action_my_experiments_feed' );
