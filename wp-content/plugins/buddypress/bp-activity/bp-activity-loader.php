 ...
 *
 * An activity stream component, for users, groups, experiments, and site tracking.
 *
 * @package BuddyPress
...
class BP_Activity_Component extends BP_Component {
	...
	public function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		// Additional menu if experiments is active
		if ( bp_is_active( 'experiments' ) ) {
			$sub_nav[] = array(
				'name'            => __( 'Experiments', 'buddypress' ),
				'slug'            => bp_get_groups_slug(),
				'parent_url'      => $activity_link,
				'parent_slug'     => $this->slug,
				'screen_function' => 'bp_activity_screen_experiments',
				'position'        => 50,
				'item_css_id'     => 'activity-experiments'
			);
		}
	...
	public function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( bp_is_active( 'experiments' ) ) {
				$wp_admin_nav[] = array(
					'parent' => 'my-account-' . $this->id,
					'id'     => 'my-account-' . $this->id . '-experiments',
					'title'  => __( 'Experiments', 'buddypress' ),
					'href'   => trailingslashit( $activity_link . bp_get_experiments_slug() )
				);
			}			
