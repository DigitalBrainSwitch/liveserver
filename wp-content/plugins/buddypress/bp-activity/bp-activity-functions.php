 ...
 *       page.
 * @uses bp_is_experiment_activity() to check if on a single experiment, the current page
 *       is the experiment activities.
 * @uses bp_is_experiment_home() to check if the current page is a single experiment home
 *       page.
 *
...
function bp_activity_do_heartbeat() {
	...
	if ( bp_is_active( 'experiments') ) {
		// If no custom front, then activities are loaded in experiment's home
		$has_custom_front = bp_locate_template( array( 'experiments/single/front.php' ), false, true );

		if ( bp_is_experiment_activity() || ( ! $has_custom_front && bp_is_experiment_home() ) ) {
			$retval = true;
		}
	}
